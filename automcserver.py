...
def setupnewserver(stdscr):
    ...
    usecustomjavaversion = cursesplus.askyesno(stdscr,["Do you want to use the default java install (program java)?",f"Version {get_java_version()}"])
    ...
    startnow = cursesplus.askyesno(stdscr,["Do you want to start your server now","to generate remaining config files","and to generate a default world?"])
...
def main(stdscr):
    ...
    try:
        ...
        if not APPDATA["hasCompletedOOBE"]:
            ...
            setupservernow = cursesplus.askyesno(stdscr,["AMCS OOBE","","Would you like to set up a server now?"])
            stdscr.clear()
            if setupservernow:
                cursesplus.cursestransition(stdscr,type=0)
